...
# ... [Everything above this remains exactly the same] ...
...
# --- 8.5 DYNAMIC JOINT MAPPING ---
# Retrieve the exact DOF names generated by the PhysX parser
dof_names = orbitron.dof_names
print(f" [INFO] Detected Joints: {dof_names}")

def get_dof_idx(keywords):
    for i, name in enumerate(dof_names):
        # Check if all keywords exist in the joint name (case-insensitive)
        if all(k in name.lower() for k in keywords):
            return i
    print(f" [WARNING] Could not find joint matching: {keywords}")
    return 0 # Fallback to prevent crash

# Map indices dynamically regardless of URDF parsing order
...
# internal CAD mirroring (the wheels are no longer fighting each other).
IDX_FR = 0 
IDX_BR = 1 
IDX_BL = 2 
IDX_FL = 3 
...
print(f" [INFO] Drive System Active. Global Kinematics Inverted.")
...
while simulation_app.is_running():
    ...
    # 1. Base Inputs (-1.0 to 1.0)
    accel = float(up - down)       
    steer = float((right - left))    

    # 2. Global Kinematic Correction (The Fix)
    # The chassis acts as a unified block, but the visual "front" of the robot 
    # was facing the opposite direction of our math. We simply multiply the 
    # entire drive vector by -1.
    left_mix = accel + steer
    right_mix = accel - steer

    # 3. Normalize to prevent saturation when combining inputs
    max_mag = max(1.0, abs(left_mix), abs(right_mix))
    
    # 4. Dispatch through the HAL
    ...
    if num_dof >= 4:
        ...
        # Dispatch to the specific joints
