...
def main(telemetry_queue, cmd_queue):
    ...
    print(f" [INFO] Orbitron Architecture: Kinematic Drop Test Booting...")
    ...
    # THE FIX: Apply the context, but intentionally leave the axis attributes blank.
    # This prevents the invalid token crash, allows the context to initialize, 
    # and defaults the vehicle to X-Forward, fixing the swapped controls.
    ...
    def setup_raycast_vehicle(root_path):
        ...
        chassis_xform = UsdGeom.Xformable(root_prim)
        translate_op = chassis_xform.GetTranslateOp()
        if not translate_op:
            translate_op = chassis_xform.AddTranslateOp()
        ...
        URDF_ANCHORS = {
            "FL_1": Gf.Vec3f(-0.271599, 0.170362, 0.06477),
            "FR_1": Gf.Vec3f(0.271599, 0.170362, 0.06477),
            "BL_1": Gf.Vec3f(-0.271599, -0.170362, 0.06477),
            "BR_1": Gf.Vec3f(0.271599, -0.170362, 0.06477)
        }
        ...
        for prim in Usd.PrimRange(root_prim):
            ...
            target_key = next((w for w in wheel_links if w in prim_name), None)
            
            if target_key and "visuals" not in str(prim.GetPath()):
                anchor_vec_f = URDF_ANCHORS[target_key]
                ...
                susp_api.CreateTravelDistanceAttr(0.1) 
                susp_api.CreateSpringStrengthAttr(50000.0) 
                ...
                wheel_controllers[target_key] = ctrl_api

        mass_api = UsdPhysics.MassAPI.Apply(root_prim)
        mass_api.CreateMassAttr(OrbitronConfig.CHASSIS_MASS_KG)
        mass_api.CreateCenterOfMassAttr(Gf.Vec3f(0.0, 0.0, 0.08))
    ...
    # THE FIX: Flipped the front two wheels so the polarities match left-to-right
    WHEEL_POLARITY = {
        "FL_1": -1.0, 
        "FR_1":  1.0, 
        "BL_1": -1.0, 
    ...
    while simulation_app.is_running():
        ...
        final_t_fl = sanitize_torque(m_torque_fl) * OrbitronConfig.GEAR_RATIO * OrbitronConfig.GEARBOX_EFFICIENCY * WHEEL_POLARITY["FL_1"]
        final_t_fr = sanitize_torque(m_torque_fr) * OrbitronConfig.GEAR_RATIO * OrbitronConfig.GEARBOX_EFFICIENCY * WHEEL_POLARITY["FR_1"]
        final_t_bl = sanitize_torque(m_torque_bl) * OrbitronConfig.GEAR_RATIO * OrbitronConfig.GEARBOX_EFFICIENCY * WHEEL_POLARITY["BL_1"]
        final_t_br = sanitize_torque(m_torque_br) * OrbitronConfig.GEAR_RATIO * OrbitronConfig.GEARBOX_EFFICIENCY * WHEEL_POLARITY["BR_1"]
